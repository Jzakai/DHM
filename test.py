...
def run_phase_difference(calledFromFunction=False):
    ...
    A1 = images_dict.get(image_label_var.get())
    print(np.mean(A1), type(A1), A1.shape)
    ...
    if not calledFromFunction:
        ...
        if type_var.get() == "1 Beam":
            im =ax.imshow(combined_clean, cmap='gray', vmin=combined_clean.min(), vmax=combined_clean.max())
...
def select_roi(calledFromFunction=False):
    ...
    fig, ax = plt.subplots()
    ax.imshow(unwrapped_psi_image, cmap='gray', vmin=unwrapped_psi_image.min(), vmax=unwrapped_psi_image.max())
    ...
    if not calledFromFunction:
        fig, ax = plt.subplots()
        im = ax.imshow(roi, cmap='gray',  vmin=roi.min(), vmax=roi.max())
...
def reduce_noise(calledFromFunction=False):
    ...
    if(calledFromFunction is False):
        ...
        fig, ax = plt.subplots(figsize=(8, 6))
        im = ax.imshow(noise_red_phase, extent=(delta_x.min(), delta_x.max(), delta_y.min(), delta_y.max()), cmap='Grays')
...
def compute_2d_thickness(calledFromFunction=False):
    ...
    if(calledFromFunction):
        ...
        fig, ax = plt.subplots(figsize=(8, 6))
        im = ax.imshow(thickness, extent=(delta_x.min(), delta_x.max(), delta_y.min(), delta_y.max()), cmap='Grays')
...
def compute_1d_thickness(calledFromFunction=False):
    ...
    fig, ax = plt.subplots()
    ax.imshow(thickness_1d, cmap='Grays')
...
def run_all():
    ...
    axs = axs.ravel()  # Flatten the 2D array of axes for easier indexing
        ...
        # 1. Original Image
    im0 = axs[0].imshow(image, cmap='gray', vmin=image.min(), vmax=image.max())
    ...
    # 2. FFT Spectrum + Mask
    im1 = axs[1].imshow(spectrum_display, cmap='gray', vmin=spectrum_display.min(), vmax=spectrum_display.max())
    ...
    # 3. Unwrapped Phase
    im2 = axs[2].imshow(unwrapped_psi_image, cmap='gray', vmin=unwrapped_psi_image.min(), vmax=unwrapped_psi_image.max())
    ...
    # 4. ROI
    im3 = axs[3].imshow(roi, cmap='gray', vmin=roi.min(), vmax=roi.max())
    ...
    im5 = axs[5].imshow(noise_red_phase, extent=(delta_x.min(), delta_x.max(), delta_y.min(), delta_y.max()),
                    cmap='gray', vmin=noise_red_phase.min(), vmax=noise_red_phase.max())
